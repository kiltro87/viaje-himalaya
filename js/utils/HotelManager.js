...
export class HotelManager {
     ...
     */
    loadHotelsData() {
        const hotelsData = tripConfig.accommodations || [];
    ...
    loadReservationsData() {
        const reservationsData = tripConfig.accommodations.flatMap(hotel => hotel.reservations || []) || [];
    ...
    async generateAccommodationExpenses() {
        ...
        for (const reservation of this.getAllReservations()) {
            ...
            const individualCost = reservation.shared && reservation.splitBetween 
                ? reservation.totalCost / reservation.splitBetween 
                : reservation.totalCost;

            const expense = {
                concept: `${hotel.name} (${reservation.checkInDate})`,
                cost: individualCost,
                category: 'Alojamiento',
                phase: hotel.country === 'Nepal' ? 'nepal' : 'butan',
                country: hotel.country
            };

            // Add expense directly to tripConfig.budgetData.budgetData.Alojamiento
            try {
                const { tripConfig } = await import('../config/tripConfig.js');
                if (!tripConfig.budgetData.budgetData.Alojamiento) {
                    tripConfig.budgetData.budgetData.Alojamiento = [];
                }
                
                // Check if expense already exists to avoid duplicates
                const existingExpense = tripConfig.budgetData.budgetData.Alojamiento.find(
                    item => item.concept === expense.concept
                );
                
                if (!existingExpense) {
                    tripConfig.budgetData.budgetData.Alojamiento.push(expense);
                    expenses.push(expense);
                    if (Logger && Logger.success) {
                        Logger.success(`Added accommodation budget item: ${expense.concept} - $${expense.cost}`);
                    }
                } else {
                    if (Logger && Logger.info) {
                        Logger.info(`Accommodation budget item already exists: ${expense.concept}`);
                    }
                }
            } catch (error) {
                if (Logger && Logger.error) {
                    Logger.error(`Failed to add accommodation budget item for ${hotel.name}:`, error);
                }
            }
        }
