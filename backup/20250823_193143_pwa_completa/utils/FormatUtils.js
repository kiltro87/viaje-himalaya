    ...
    static getTripStartDate() {
        ...
        if (tripConfig.flightsData && tripConfig.flightsData.length > 0) {
            ...
            if (firstFlight.segments && firstFlight.segments.length > 0) {
                try {
                    // La estructura usa fromDateTime: '9 de Octubre 22:45'
                    const fromDateTime = firstFlight.segments[0].fromDateTime;
                    
                    // Parsear fecha en formato español "9 de Octubre 22:45"
                    const months = {
                        'Enero': 0, 'Febrero': 1, 'Marzo': 2, 'Abril': 3, 'Mayo': 4, 'Junio': 5,
                        'Julio': 6, 'Agosto': 7, 'Septiembre': 8, 'Octubre': 9, 'Noviembre': 10, 'Diciembre': 11
                    };
                    
                    const parts = fromDateTime.split(' ');
                    const day = parseInt(parts[0]);
                    const monthName = parts[2];
                    const monthIndex = months[monthName];
                    
                    if (monthIndex !== undefined) {
                        const departureDate = new Date(APP_CONFIG.TRIP_START_YEAR, monthIndex, day);
                        Logger.data('Trip start date derived from first flight', { 
                            original: fromDateTime,
                            parsed: departureDate 
                        });
                        return departureDate;
                    }
                } catch (e) {
                    Logger.error('Error parsing flight departure date, using fallback', { error: e.message });
                }
            }
        }
        
        // Fecha de fallback
        Logger.warning('No flight data found or invalid, using default trip start date.');
        return APP_CONFIG.DEFAULT_FALLBACK_DATE;
    }
